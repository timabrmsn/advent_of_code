...
escaped=$(sed -e 's/\\/\\\\/g' -e 's/"/\\"/g' < input.txt | # replace all '\' with '\\' and '"' with '\"'
gawk '{ printf "\x22"$0"\x22" }' | wc -c)                   # print surrounding '"' and get "escaped" character count
code=$(tr -d '\n' < input.txt | wc -c)                      # re-calculate "code" character count
echo $escaped - $code | bc                                  # do the math