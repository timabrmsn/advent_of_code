...
BEGIN {
  ...
  map[0","0]++         # visit initial house in array
}
  /\^/ { y++ }         # if north: increment y
  /v/  { y-- }         # if south: decrement y
  />/  { x++ }         # if east: increment x
  /</  { x-- }         # if west: decrement x
  { map[x","y]++ }     # visit house for every line of input
END {
