...
gawk -F x '{                    # define "x" as field separator
  side[1] = $1 * 2              # stick 2x each side in an array
  side[2] = $2 * 2
  side[3] = $3 * 2
  asort(side)                   # sort array
  total += ($1 * $2 * $3)       # add volume of box to total
  total += (side[1] + side[2])  # add 2x each of the two smallest sides for bow
}
END {
  print total
}' input.txt