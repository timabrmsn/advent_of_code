...
gawk -F x '{                        # define "x" as field separator
  side[1] = $1*$2                   # stick size of each side in an array
  side[2] = $2*$3
  side[3] = $3*$1
  asort(side)                       # sort array by values
  total += (2*l*w + 2*w*h + 2*h*l)  # add area of box to total
  total += side[1]                  # add smallest side for slack
  } 
END { 
  print total 
}' input.txt